...
use crate::input;
use crate::output::{NavAction, NavWidget};

use super::super::events::ServoCommand;
...
///
/// Returns `true` if the event was a scroll (so the caller can decide whether
/// to trigger a text extraction).
pub fn handle_input(
    event: input::Event,
...
/// Drain all immediately-available input events from `event_rx`, dispatching
/// each one. Returns `true` if any of them was a scroll event.
pub fn drain_pending_inputs(
    event_rx: &mpsc::Receiver<crate::browser::servo::events::RuntimeEvent>,
    servo_tx: &mpsc::SyncSender<ServoCommand>,
    app: &mut AppState,
) -> Result<bool> {
    let mut any_scroll = false;

    while let Ok(crate::browser::servo::events::RuntimeEvent::Input(event)) = event_rx.try_recv() {
